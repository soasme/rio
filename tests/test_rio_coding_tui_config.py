...
from rio_coding.tui.config import (
    HIGH_CONTRAST_THEME,
    TuiConfigError,
    TuiKeybindings,
    TuiSettings,
...
def test_tui_settings_path_uses_rio_home(tmp_path: Path) -> None:
    paths = RioPaths(home=tmp_path / ".rio", agents_home=tmp_path / ".agents")

    assert tui_settings_path(paths) == tmp_path / ".rio" / "tui.json"
...
def test_load_tui_settings_reads_keybindings(tmp_path: Path) -> None:
    ...
    assert settings.keybindings.toggle_state_panel == "ctrl+s"
...
def test_save_tui_settings_writes_json(tmp_path: Path) -> None:
    ...
    path = save_tui_settings(TuiSettings(theme="rio-light"), paths)

    assert path == tmp_path / ".rio" / "tui.json"
    assert load_tui_settings(paths).theme == "rio-light"
...
def test_tui_settings_ignore_unknown_fields() -> None:
    ...
    assert settings.theme == "rio-light"
...
def test_tui_settings_accept_unknown_theme_and_fall_back_when_resolving() -> None:
    ...
    assert settings.resolved_theme == get_tui_theme("rio-dark")
...
def test_tui_settings_accept_light_theme() -> None:
    settings = tui_settings_from_json({"theme": "rio-light"})
...
def test_tui_keybindings_serialize_to_json() -> None:
    ...
    assert settings.to_json()["keybindings"]["toggle_state_panel"] == "ctrl+s"
...
def test_get_tui_theme_returns_builtin_theme() -> None:
    ...
    assert get_tui_theme("rio-light").prompt_border == "#2563eb"
    assert get_tui_theme("rio-dark").screen_background == "#000000"
