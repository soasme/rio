...
to `rio` throughout (module paths, `TauPaths` -> `RioPaths`, `.tau` -> `.rio`,
env var prefixes). Provider ids, model ids, base URLs, and thinking-mode values
are kept exactly as tau has them since rio's bundled catalog.toml is an
unmodified copy of tau's.
...
def test_stale_preferences_cannot_restore_removed_codex_alias(tmp_path: Path) -> None:
    ...
    rio_home.mkdir()
    (rio_home / "providers.json").write_text(
        json.dumps(
            {
                "schema_version": 2,
    ...
    settings = load_provider_settings(RioPaths(home=rio_home))
...
def test_load_provider_settings_accepts_huggingface_inference_provider_preferences(
    ...
    provider = load_provider_settings(RioPaths(home=rio_home)).get_provider("huggingface")
...
def test_load_provider_settings_accepts_provider_preferences_with_user_catalog(
    ...
    rio_home = tmp_path / ".rio"
    rio_home.mkdir()
    (rio_home / "catalog.toml").write_text(
        """
schema_version = 1
...
def test_save_and_load_provider_settings_round_trip(
    ...
    assert path == tmp_path / ".rio" / "providers.json"
...
# ---------------------------------------------------------------------------
# Runtime config construction (openai-compatible / anthropic)
# ---------------------------------------------------------------------------
...
def test_kimi_k3_maps_thinking_levels_to_reasoning_effort(
    ...
    settings = load_provider_settings(RioPaths(home=Path("/missing")))
...
def test_huggingface_glm_5_2_does_not_send_unverified_medium_effort(
    ...
    monkeypatch.setenv("HF_TOKEN", "test-key")
    provider = load_provider_settings(RioPaths(home=Path("/missing"))).get_provider("huggingface")
...
def test_load_provider_settings_migrates_custom_provider_to_catalog(
    ...
    (rio_home / "providers.json").write_text(json.dumps(original), encoding="utf-8")
...
""".strip(),
    ...
    store = FileCredentialStore(rio_home / "credentials.json")
...
# ---------------------------------------------------------------------------
# create_model_provider (runtime provider construction)
# ---------------------------------------------------------------------------
...
def test_create_model_provider_uses_anthropic_oauth_runtime_auth(tmp_path: Path) -> None:
    store = FileCredentialStore(tmp_path / "credentials.json")
    store.set_oauth(
        "anthropic",
        OAuthCredential(
...
def test_anthropic_api_key_auth_keeps_the_default_cache_retention(tmp_path: Path) -> None:
...
def test_provider_compat_can_re_enable_cache_breakpoints_on_a_gateway(tmp_path: Path) -> None:
...
def test_provider_compat_can_clamp_the_one_hour_ttl_on_oauth(tmp_path: Path) -> None:
...
def test_provider_compat_can_suppress_only_the_tools_breakpoint(tmp_path: Path) -> None:
...
def test_copilot_anthropic_protocol_models_disable_cache_breakpoints(tmp_path: Path) -> None:
...
def test_copilot_model_metadata_compat_can_enable_cache_breakpoints(tmp_path: Path) -> None:
...
def test_create_model_provider_uses_model_max_tokens_for_anthropic_protocol_model(
    ...
    provider_config_value = replace(catalog_config, model_metadata=metadata)
...
def test_create_model_provider_uses_copilot_token_base_url(tmp_path: Path) -> None:
    store = FileCredentialStore(tmp_path / "credentials.json")
    store.set_oauth(
        "github-copilot",
        OAuthCredential(
...
def test_create_model_provider_rejects_model_not_declared_for_provider(tmp_path: Path) -> None:
    store = FileCredentialStore(tmp_path / "credentials.json")
    provider_config_value = OpenAICompatibleProviderConfig(
        name="local",
        models=("qwen",),
        default_model="qwen",
    )
    ...
    with pytest.raises(
        ...
        create_model_provider(provider_config_value, credential_store=store, model="llama")


def test_create_model_provider_maps_codex_reasoning_effort_like_pi(tmp_path: Path) -> None:
    store = FileCredentialStore(tmp_path / "credentials.json")
    provider_config_value = OpenAICodexProviderConfig(
        thinking_levels=("off", "minimal", "low", "medium", "high", "xhigh"),
        thinking_models=("gpt-5.5",),
        thinking_parameter="reasoning.effort",
    )
...
def test_create_model_provider_coerces_unsupported_startup_thinking_level(
    ...
    monkeypatch.setenv("RIO_TEST_KIMI_CODE_API_KEY", "test-key")
    store = FileCredentialStore(tmp_path / "credentials.json")
    provider_config_value = OpenAICompatibleProviderConfig(
        name="kimi-code",
        api_key_env="RIO_TEST_KIMI_CODE_API_KEY",
        models=("k3",),
        default_model="k3",
        thinking_levels=("low", "medium", "high", "xhigh"),
        thinking_default="xhigh",
    ...
    provider = create_model_provider(
        ...
        credential_store=store,
        model="k3",
        thinking_level=resolve_startup_thinking_level(provider_config_value, "k3"),
    )
...
# ---------------------------------------------------------------------------
# OAuth runtime credential resolver (ported from test_oauth_providers.py)
# ---------------------------------------------------------------------------


async def test_runtime_oauth_resolver_refreshes_and_persists_atomically(tmp_path: Path) -> None:
...
async def test_runtime_oauth_resolver_spends_a_refresh_token_once(tmp_path: Path) -> None:
...
def test_switching_provider_and_model_preserves_configuration_and_credentials(
    tmp_path: Path,
) -> None:
    paths = RioPaths(home=tmp_path / ".rio")
    credentials = FileCredentialStore(tmp_path / ".rio" / "credentials.json")
    credentials.set_api_key("openai", "sk-openai-test")
    credentials.set_oauth(
        "anthropic",
        OAuthCredential(access="anthropic-access", refresh="anthropic-refresh", expires=1),
    )

    settings = load_provider_settings(paths)
    settings = save_default_provider_model(
    ...
    # Switch the active provider/model to anthropic.
    switched = save_default_provider_model(
        provider_name="anthropic",
        model="claude-sonnet-4-6",
        paths=paths,
    )

    assert switched.default_provider == "anthropic"
    assert switched.get_provider("anthropic").default_model == "claude-sonnet-4-6"

    # The provider we switched away from keeps its own configuration exactly
    # as it was -- switching carries no transcript, so there is nothing to
    # translate or lose for either provider.
    openai_after_switch = switched.get_provider("openai")
    assert openai_after_switch.default_model == "gpt-5.4"

    # Credentials for both providers -- including the one no longer active --
    # remain intact and independently resolvable.
    openai_config = openai_compatible_config_from_provider(
        openai_after_switch,
        credential_reader=credentials,
    )
    assert openai_config.api_key == "sk-openai-test"

    anthropic_provider = switched.get_provider("anthropic")
    assert isinstance(anthropic_provider, AnthropicProviderConfig)
    anthropic_credential = credentials.get_oauth("anthropic")
    assert anthropic_credential is not None
    assert anthropic_credential.access == "anthropic-access"

    # Switching back and forth again is idempotent for the untouched provider.
    switched_back = save_default_provider_model(
        provider_name="openai",
        model="gpt-5.4",
        paths=paths,
    )
    assert switched_back.default_provider == "openai"
    assert switched_back.get_provider("anthropic").default_model == "claude-sonnet-4-6"
    assert credentials.get_oauth("anthropic") == anthropic_credential
