...
def test_install_local_extension_file_is_discovered(tmp_path: Path) -> None:
    ...
    source.write_text("def setup(rio):\n    pass\n", encoding="utf-8")
    ...
    paths = RioResourcePaths(root=tmp_path / "home", cwd=tmp_path)
...
def test_install_rejects_directory_that_would_not_be_auto_discovered(
    ...
    entry.parent.mkdir(parents=True)
    entry.write_text("def setup(rio):\n    pass\n", encoding="utf-8")
...
@pytest.mark.parametrize(
    ...
    [
        (
            "git:github.com/rian-dolphin/rio-subagents@v1.2.0",
            "https://github.com/rian-dolphin/rio-subagents",
            "v1.2.0",
            "rio-subagents",
        ),
        (
            "https://github.com/rian-dolphin/rio-subagents.git",
            "https://github.com/rian-dolphin/rio-subagents.git",
            None,
            "rio-subagents",
        ),
        (
            "git:git@github.com:rian-dolphin/rio-subagents.git@abc123",
            "git@github.com:rian-dolphin/rio-subagents.git",
            "abc123",
            "rio-subagents",
        ),
    ],
...
def test_missing_git_is_reported_as_install_error(tmp_path: Path) -> None:
    ...
    with pytest.raises(ExtensionInstallError, match="git executable not found"):
        install_extension(
            "git:github.com/example/rio-demo",
            extensions_dir=tmp_path / "extensions",
...
def test_install_git_source_clones_and_checks_out_ref(tmp_path: Path) -> None:
    ...
    destination = install_extension(
        "git:github.com/example/rio-demo@v1",
        extensions_dir=tmp_path / "extensions",
        command_runner=fake_run,
    )

    assert destination == tmp_path / "extensions" / "rio-demo"
    assert calls[0][:4] == ["git", "clone", "--", "https://github.com/example/rio-demo"]
