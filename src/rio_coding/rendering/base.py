...
What a renderer consumes instead is `rio_coding.events.CodingSessionEvent`:
the per-step skill lifecycle events `rio_agent` emits, plus the
session-level events `rio_coding.events` defines.
"""
...
from rio_coding.events import CodingSessionEvent
...
class PrintOutputMode(StrEnum):
    ...
    by `rio_coding.rendering.steps`, which does not implement `EventRenderer`
