"""Project instruction discovery for rio coding skills."""
...
def discover_project_context(
    ...
    """Discover project instruction files for skill instruction context."""
...
def _context_file_candidates(paths: RioResourcePaths) -> tuple[Path, ...]:
    ...
    if paths.cwd is not None and paths.project_resources_enabled:
        ...
        rio_paths = paths._paths()
        candidates.extend(
            [
                rio_paths.project_rio_dir(cwd) / "AGENTS.md",
                rio_paths.project_agents_dir(cwd) / "AGENTS.md",
            ]
        )
