...
def load_prompt_templates(paths: RioResourcePaths | None = None) -> list[PromptTemplate]:
    """Load markdown prompt templates from rio and `.agents` resource directories."""
    resource_paths = paths or RioResourcePaths()
...
def load_prompt_templates_with_diagnostics(
    paths: RioResourcePaths | None = None,
...
def parse_prompt_template_arguments(text: str) -> list[str]:
    """Parse prompt invocation arguments using shell-style simple quote rules."""
    arguments: list[str] = []
    current: list[str] = []
    quote: str | None = None
...
def substitute_prompt_template_args(content: str, arguments: Sequence[str]) -> str:
    """Substitute shell-compatible positional and aggregate prompt arguments."""
...
def expand_prompt_template_command(
    ...
    shell-compatible `$1`, `$2`, `$@`, and `$ARGUMENTS` placeholders, including
    default values and simple slices. Legacy `{{ arguments }}` and `{{ args }}`
    placeholders remain supported. If a template has no argument placeholder,
    arguments are appended after a blank line.
    """
    stripped = text.strip()
