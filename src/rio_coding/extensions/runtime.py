...
import rio_coding.built_in_extensions as built_in_extension_registry
...
from rio_coding.built_in_extensions import BuiltInExtension, BuiltInExtensionContext
...
from rio_coding.credentials import CredentialStore, FileCredentialStore, credentials_path
from rio_coding.extensions.api import (
    AGENT_EVENT_TYPES,
...
from rio_coding.extensions.loader import (
...
from rio_coding.extensions.providers import CredentialReader, DynamicProvider
from rio_coding.local_backends import LocalBackend, LocalBackendRegistry
from rio_coding.paths import RioPaths
from rio_coding.project_trust import ExtensionTrustResult, ProjectTrustEvent
from rio_coding.provider_config import ProviderConfig
from rio_coding.resources import ResourceDiagnostic, RioResourcePaths
from rio_coding.system_prompt import PromptSection
...
class ExtensionRuntime:
    ...
    def __init__(
        ...
        *,
        ...
        built_in_paths = paths or RioPaths()
    ...
    def reset_for_reload(self) -> None:
        ...
        parity): any `rio` API object, context, or ui facade captured before
    ...
    async def emit_event(self, event: object) -> None:
        ...
        if not isinstance(event_type, str):
            ...
            if event_type not in AGENT_EVENT_TYPES:
                raise TypeError("Extension events must expose a recognized event type")
        ...
        for owner, handler in handlers:
            try:
                await _resolve(handler(deepcopy(event), self._fresh_context(owner.source_id)))
            except Exception as exc:  # noqa: BLE001 - extensions are an isolation boundary
