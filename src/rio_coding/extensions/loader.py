...
_MODULE_NAME_PREFIX = "rio_extension"
...
def extension_dirs(
    ...
    *,
    ...
    Project extensions are opt-in (`--project-extensions`) until a project
    trust decision has been made, because they execute at session startup.
    """
    dirs: list[Path] = []
    if include_project_dir and paths.cwd is not None and paths.project_resources_enabled:
        dirs.append(paths.cwd / ".rio" / "extensions")
    if include_user_dir:
...
def discover_extensions(
    ...
    *,
    ...
    if include_resource_dirs:
        ...
        if include_project_dir and paths.cwd is not None and paths.project_resources_enabled:
            directories.append((paths.cwd / ".rio" / "extensions", "project"))
        if include_user_dir:
...
def _manifest_entries(
    ...
    """Resolve entry files declared in `<dir>/pyproject.toml` under `[tool.rio]`.
    ...
    use package.json manifest"); `tool.rio.extensions` is the Python-shaped
    ...
    rio_table = tool_table.get("rio") if isinstance(tool_table, dict) else None
    declared = rio_table.get("extensions") if isinstance(rio_table, dict) else None
    if declared is None:
        return []
    if not isinstance(declared, list) or not all(isinstance(item, str) for item in declared):
        diagnostics.append(
            ResourceDiagnostic(
                kind="extension",
                path=manifest,
                message="`tool.rio.extensions` must be a list of file paths",
                severity="error",
            )
        )
        return []
    ...
    if not callable(setup):
        ...
        return None, [
            _error_diagnostic(
                ...
                f"`{EXTENSION_ENTRY_ATTRIBUTE}(rio)` entry point",
            )
        ]
