...
from rio_coding.built_in_extensions import BuiltInExtensionContext
...
def setup(
    ...
    dependencies = context or BuiltInExtensionContext(
        paths=RioPaths(),
        credential_store=FileCredentialStore(credentials_path(RioPaths())),
    ...
    service = LlamaCppService(
        ...
        register_provider=rio.register_provider,
        update_provider=rio.update_provider,
        register_backend=rio.register_local_backend,
    )
    rio.register_provider(service.provider())
    rio.register_local_backend(service.backend())
