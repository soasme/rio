...
from rio_ai.env import DEFAULT_OPENAI_COMPATIBLE_TIMEOUT_SECONDS, OpenAICompatibleConfig
from rio_ai.http import create_async_client
from rio_ai.messages import TextContent, UserMessage
from rio_ai.openai_compatible import OpenAICompatibleProvider
from rio_ai.provider import CancellationToken
from rio_ai.provider_events import (
    AssistantErrorEvent,
    AssistantStartEvent,
    ToolCallEndEvent,
)
...
from rio_coding.local_backends import (
    LocalAction,
    LocalArtifactOption,
    LocalBackend,
    LocalBackendStatus,
...
class LlamaCppService:
    ...
    def __init__(
        ...
        *,
        ...
        self.credential_store: CredentialStore = (
            ...
            else FileCredentialStore(credentials_path(RioPaths()))
        )
    ...
    async def status(self, context: LocalOperationContext) -> LocalBackendStatus:
        ...
        if not self.configured:
            ...
            return self._status(
                ...
                diagnostics=(
                    LocalDiagnostic(
                        "Rio will check the offered default endpoint when this backend opens. "
        ...
        results = tuple(
            LocalSearchResult(
                ...
                diagnostic=(
                    ...
                    "Rio's search token is never forwarded."
                    if repository.gated
                    else None
                ),
            )
            ...
            return LocalOperationResult(
                ...
                diagnostics=(
                    LocalDiagnostic(
                        ...
                        "Refresh before retrying; Rio will not replay the mutation.",
                        "warning",
                        "reconciliation",
                    ),
                ),
            ...
            return LocalOperationResult(
                ...
                diagnostics=(
                    LocalDiagnostic(
                        ...
                        "retrying; Rio will not replay the interrupted mutation.",
                        "error",
                        "reconciliation",
                    ),
                ),
    ...
    async def _probe_tools(
        ...
        try:
            async for event in provider.stream_response(
                model=model,
                system="Call the rio_probe tool.",
                messages=[UserMessage(content="Call rio_probe now.")],
                tools=[tool],
                signal=signal,
            ):
...
def _reported_context_window(item: Mapping[str, object]) -> int | None:
    # These names are accepted only when directly reported by a server. Rio
