"""Runtime provider construction for rio coding sessions."""
...
from rio_ai.env import AnthropicConfig, OpenAICompatibleConfig, RuntimeProviderAuth
...
from rio_coding.oauth_types import OAuthProvider
...
from rio_coding.thinking import ThinkingLevel, normalize_thinking_level, reasoning_effort_for_level

if TYPE_CHECKING:
    from rio_coding.extensions.providers import (
        CredentialReader,
        DynamicProvider,
        ProviderModel,
...
class ClosableModelProvider(ModelProvider, Protocol):
    """Runtime provider object rio owns and can close."""
...
class DeferredModelProvider:
    """Construct a provider on its first request, allowing login before use.

    Failed construction is not cached: a later request can use credentials
    saved since the previous attempt. The factory should read current settings.
    """

    def __init__(self, factory: Callable[[], ClosableModelProvider]) -> None:
        self._factory = factory
        self._provider: ClosableModelProvider | None = None

    async def stream_response(self, **kwargs):
        if self._provider is None:
            self._provider = self._factory()
        async for event in self._provider.stream_response(**kwargs):
            yield event
...
async def create_dynamic_model_provider(
    ...
    *,
    ...
    Dynamic definitions remain process-local and are supplied by loaded extensions.
    """
    from rio_coding.extensions.providers import (
...
async def _resolve_dynamic_runtime_auth(
    ...
    *,
    ...
    try:
        ...
        # Preserve only rio's exact strategy and host-authored missing-key error.
