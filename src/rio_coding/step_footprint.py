...
and the latest observation (see ``rio_agent.prompt.build_step_messages`` and
``rio_agent.loop.run_skill_loop``). There is nothing to compact, so only the
...
def estimate_step_footprint(
    *,
    ...
    Mirrors what ``rio_agent.prompt.build_step_messages`` actually sends to
