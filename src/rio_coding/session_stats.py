...
reasoning) is discarded by ``rio_agent.loop`` before the loop's caller ever
...
of ``rio_coding.session_usage``'s per-step footprint estimates instead of
...
in this port already go through ``rio_coding.provider_catalog`` via
...
from rio_coding.session_store.entries import SessionEntry, TurnEntry, ValidationFailureEntry
from rio_coding.session_usage import collect_session_usage
