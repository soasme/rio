...
def load_skills(paths: RioResourcePaths | None = None) -> list[Skill]:
    """Load markdown skills from rio and `.agents` resource directories.
...
def parse_skill_invocation(text: str) -> SkillInvocation | None:
    """Parse rio's expanded skill invocation message format."""
    match = re.match(
...
def build_skill_index(skills: Sequence[Skill]) -> str:
    """Build a concise index of available skills for future prompt assembly."""
...
def _load_skills_from_dir_with_diagnostics(
    ...
    # rio follows the Agent Skills spec everywhere: a skill is a directory
    ...
    # tells the user how to migrate. This intentionally diverges from tools
    # that keep a permissive "any .md is a skill" path purely for backward
    # compatibility.
    skills: list[Skill] = []
    diagnostics: list[ResourceDiagnostic] = []
    seen: set[str] = set()
