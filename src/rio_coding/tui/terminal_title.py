"""Helpers for updating the terminal window/tab title from Rio's TUI."""
...
def terminal_title_supported(
    *,
    ...
    """Return whether Rio should emit OSC title sequences in this process."""
    env = os.environ if environ is None else environ
    if env.get("RIO_TERMINAL_TITLE", "").lower() in {"0", "false", "no", "off"}:
        return False
    ...
    if env.get("TERM", "") == "dumb":
        return False
    return not (env.get("CI", "") and env.get("RIO_TERMINAL_TITLE", "").lower() != "1")
...
def build_terminal_title(
    ...
    *,
    ...
    """Return Rio's terminal tab title for the current session/running state."""
    title = sanitize_terminal_title(session_title)
    title = (
        RIO_TITLE_MARK
        if not title or title.lower() == "untitled session"
        else f"{RIO_TITLE_MARK} | {title}"
    )
    if not running:
        return title
...
class TerminalTitleController:
    ...
    def update(self, session_title: str | None, *, running: bool, frame: int = 0) -> None:
        """Write the current Rio title if it differs from the last emitted title."""
        if not self.enabled:
            return
    ...
    def restore(self) -> None:
        """Leave the terminal title in a neutral idle Rio state on shutdown."""
        if not self.enabled:
            return
