"""JSON-defined TUI themes for rio.
...
``~/.rio/themes/*.json`` and ``<project>/.rio/themes/*.json``.
"""
...
class TuiRoleStyle:
    """Colors for one step-stream role block."""
...
@dataclass(frozen=True, slots=True)
class TuiTheme:
    """Resolved visual theme for rio's built-in Textual frontend."""
...
# Single source of truth for step-stream roles: the theme schema requires a
# style for each. rio has no append-only transcript, so these roles describe
# the SKILL.state step stream instead of chat turns:
#   * "user" -- the observation that started a turn
#   * "step" -- one committed step (StepEntryWidget)
#   * "reasoning" -- a step's discarded reasoning (ReasoningWidget)
#   * "validation_retry" -- a rejected proposal and its retry (ValidationRetryWidget)
#   * "error" -- a failed action or a non-null `last_error` state field
#   * "status" -- session-level notices (queue changes, retries, settled)
#   * "custom" -- slash-command output and anything else
StepStreamRole = Literal[
    "user",
    "step",
    "reasoning",
    "validation_retry",
...
STEP_STREAM_ROLES: tuple[str, ...] = get_args(StepStreamRole)
...
def _parse_roles(
    ...
    if not isinstance(value, dict):
        ...
        return {}
    missing = [role for role in STEP_STREAM_ROLES if role not in value]
    if missing:
        problems.append("roles missing: " + ", ".join(missing))
    unknown = sorted(set(value) - set(STEP_STREAM_ROLES))
    if unknown:
        problems.append("roles unknown: " + ", ".join(unknown))
    role_styles: dict[str, TuiRoleStyle] = {}
    for role in STEP_STREAM_ROLES:
        ...
        # Body colors also feed Textual's styles.color/background in widgets,
        # so both style components must satisfy Textual too.
...
RIO_DARK_THEME = _load_builtin_theme("rio-dark.json")
RIO_LIGHT_THEME = _load_builtin_theme("rio-light.json")
HIGH_CONTRAST_THEME = _load_builtin_theme("high-contrast.json")

_BUILTIN_THEMES: dict[str, TuiTheme] = {
    theme.name: theme for theme in (RIO_DARK_THEME, RIO_LIGHT_THEME, HIGH_CONTRAST_THEME)
}
...
def get_tui_theme(name: TuiThemeName = "rio-dark") -> TuiTheme:
...
def textual_theme_for_tui_theme(theme_name: TuiThemeName) -> Theme:
    """Map a rio theme to Textual's native theme type."""
...
def theme_css_variables(theme: TuiTheme) -> dict[str, str]:
    """Return Textual CSS variables for a resolved rio theme."""
    return {
        "rio-screen-background": theme.screen_background,
        "rio-screen-text": theme.screen_text,
        "rio-chrome-background": theme.chrome_background,
        "rio-chrome-text": theme.chrome_text,
        "rio-muted-text": theme.muted_text,
        "rio-sidebar-background": theme.sidebar_background,
        "rio-border": theme.border,
        "rio-transcript-background": theme.transcript_background,
        "rio-prompt-background": theme.prompt_background,
        "rio-prompt-text": theme.prompt_text,
        "rio-prompt-border": theme.prompt_border,
        "rio-autocomplete-background": theme.autocomplete_background,
        "rio-accent": theme.accent,
        "rio-step-running": theme.role_styles["step"].border,
        "rio-highlight-background": theme.highlight_background,
        "rio-highlight-text": theme.highlight_text,
        "rio-markdown-highlight": theme.markdown_heading,
        "rio-markdown-table-header": theme.markdown_table_header,
        "rio-markdown-table-border": theme.markdown_table_border,
        "rio-markdown-inline-code": theme.markdown_inline_code,
        "rio-markdown-code-block-background": theme.markdown_code_block_background,
        "rio-markdown-link": theme.markdown_link,
        "rio-markdown-bullet": theme.markdown_bullet,
