"""Durable Textual TUI configuration for Rio."""
...
from rio_coding.tui.themes import (
...
class TuiConfigError(ValueError):
    """Raised when Rio TUI configuration is invalid."""


@dataclass(frozen=True, slots=True)
class TuiKeybindings:
    """Configurable keys for Rio's built-in Textual frontend."""
...
@dataclass(frozen=True, slots=True)
class TuiSettings:
    """Rio TUI settings loaded from Rio home."""

    keybindings: TuiKeybindings = field(default_factory=TuiKeybindings)
    theme: TuiThemeName = "rio-dark"
    ...
    def resolved_theme(self) -> TuiTheme:
        """Return the selected theme, falling back to rio-dark when unknown."""
        try:
            return get_tui_theme(self.theme)
        except KeyError:
            return RIO_DARK_THEME


def tui_settings_path(paths: RioPaths | None = None) -> Path:
    """Return the durable TUI settings path."""
    return (paths or RioPaths()).home / "tui.json"


def load_tui_settings(paths: RioPaths | None = None) -> TuiSettings:
...
def save_tui_settings(settings: TuiSettings, paths: RioPaths | None = None) -> Path:
...
def tui_settings_from_json(data: dict[str, Any]) -> TuiSettings:
    ...
    # Ignore settings added by newer Rio versions so sharing this user-level
    ...
    return TuiSettings(
        ...
        theme=_theme_name(data.get("theme", "rio-dark")),
