"""Slash command registry for rio coding sessions.
...
from rio_coding.prompt_templates import PromptTemplate
from rio_coding.provider_catalog import BUILTIN_PROVIDER_CATALOG, builtin_provider_entry
...
from rio_coding.system_prompt import ProjectContextFile
...
def create_default_command_registry() -> CommandRegistry:
    """Create rio's built-in slash command registry."""
    registry = CommandRegistry()
    registry.register(
        SlashCommand(
            name="quit",
    ...
    registry.register(
        SlashCommand(
            name="system",
            usage="/system",
            description="Show the active skill instructions without saving them.",
            handler=_system_command,
            search_terms=("prompt", "instructions", "skill"),
        )
    )
    registry.register(
        SlashCommand(
    ...
    registry.register(
        SlashCommand(
            name="thinking",
            usage="/thinking [mode]",
            description="Show or set the reasoning-effort mode.",
    ...
    registry.register(
        SlashCommand(
            ...
            search_terms=("agents.md", "project"),
        )
    )
    registry.register(
        SlashCommand(
            name="resources",
            usage="/resources",
            description="Summarize loaded resources and diagnostics.",
...
def _name_command(context: CommandContext) -> CommandResult:
    ...
    return CommandResult(handled=True, session_name=name, message=f"Session renamed: {name}")
...
def format_reload_summary(summary: CodingReloadSummary) -> str:
    """Render a reload summary.

    The instructions line says "next step", not "next turn": a rebuilt skill
    specification takes effect on the very next step, because no history was
