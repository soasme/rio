"""Install rio extensions from local paths or Git repositories."""
...
def install_extension(
    ...
    *,
    ...
    """Install one extension into rio's user extension directory.

    Local Python files are copied. Local package directories and Git
    repositories must expose ``extension.py`` or ``[tool.rio].extensions`` so
    the normal user-directory discovery path can load them on the next run.
    """
    destination_root = extensions_dir or RioPaths().user_extensions_dir
    ...
    try:
        ...
        # Reproduce the exact ~/.rio/extensions/<name> shape during validation.
...
def parse_git_extension_source(source: str) -> GitExtensionSource:
    """Normalize a Pi-style Git source accepted by ``rio install``."""
...
def _validate_staged_extension(staging_root: Path) -> None:
    paths = RioResourcePaths(root=staging_root, cwd=Path.cwd())
    ...
    if not discovered:
        raise ExtensionInstallError(
            ...
            "[tool.rio].extensions in pyproject.toml"
        )
