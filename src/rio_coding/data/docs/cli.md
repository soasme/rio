...
rio drives one `rio_coding.session.CodingSession` from a small number of frontends: an interactive TUI, a one-shot print mode, and a JSONL RPC mode for editor/tool integrations. The CLI entry point is `rio_coding.cli:app` (script name `rio`).
...
`-t/--thinking LEVEL` sets the initial reasoning-effort level for a run (`off`, `minimal`, `low`, `medium`, `high`, `xhigh`, `max`; see `rio_coding.thinking`). An unsupported level for the selected model is an error listing the levels that model supports.
