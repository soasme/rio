...
filesystem/shell behavior outside the reusable `rio_ai` package.

Under SKILL.state (see `rio_agent`) a tool's result becomes the *entire* next
...
from rio_ai.messages import ImageContent, TextContent
...
def _with_header(header: str, text: str) -> str:
    """Prefix `text` with `header`, keeping a bare header when `text` is empty."""
...
def create_read_tool_definition(
    *,
    ...
    returned text opens with a `read <path> (...)` header describing what was
    read.
    """
    root = Path.cwd() if cwd is None else Path(cwd)
    ...
    async def execute(
        ...
        if source_mime_type is not None:
            ...
            header = _tool_header("read", str(path))
            ...
            return AgentToolResult(
                content=[
                    TextContent(
                        text=_with_header(
                            header, f"Read image file [{processed.mime_type}]{note_lines}"
                        )
                    ),
        ...
        if truncation.first_line_exceeds_limit:
            ...
            header = _tool_header("read", str(path), f"line {start_display} exceeds output limit")
            output = (
            ...
            end_display = start_line + user_limited_lines
            header = _tool_header(
                "read", str(path), f"lines {start_display}-{end_display} of {len(all_lines)}"
            )
            output = (
...
def _omitted_image_result(
    *,
    ...
    header = _tool_header("read", str(path))
    text = f"Read image file [{source_mime_type}]\n[Image omitted: {reason}.]"
    return AgentToolResult(
        content=[TextContent(text=_with_header(header, text))],
        details={
            "path": str(path),
            "source_mime_type": source_mime_type,
            "bytes": source_bytes,
...
def create_write_tool_definition(*, cwd: str | Path | None = None) -> ToolDefinition:
    ...
    written in `data`, and the returned text opens with a `write <path> (...)`
    header.
    """
    root = Path.cwd() if cwd is None else Path(cwd)
    ...
    async def execute(
        ...
        header = _tool_header("write", str(path), f"{len(content)} characters")
        return AgentToolResult(
            content=[TextContent(text=_with_header(header, f"Successfully wrote to {path}."))],
...
def create_edit_tool_definition(*, cwd: str | Path | None = None) -> ToolDefinition:
    ...
    diff, a unified patch, and the first changed line in `data`. The returned
    text opens with an `edit <path> (...)` header.
    """
    root = Path.cwd() if cwd is None else Path(cwd)
    ...
    async def execute(
        ...
        header = _tool_header("edit", str(path), f"{len(edits)} edit(s)")
        return AgentToolResult(
            content=[
                TextContent(
                    text=_with_header(
                        header, f"Successfully replaced {len(edits)} block(s) in {path}."
                    )
                )
            ],
            details={
...
def create_bash_tool_definition(
    *,
    ...
    duration, truncation metadata, and full-output path metadata. The returned
    text opens with a `bash <command> (...)` header.
    """
    root = Path.cwd() if cwd is None else Path(cwd)
    ...
    async def execute(
        ...
        if len(command_preview) > 80:
            command_preview = f"{command_preview[:77]}..."
        detail_parts: list[str] = []
        if exit_code is not None:
            detail_parts.append(f"exit {exit_code}")
        if timed_out:
            detail_parts.append("timed out")
        if cancelled:
            detail_parts.append("cancelled")
        header = _tool_header(
            "bash", command_preview, ", ".join(detail_parts) if detail_parts else None
        )

        return AgentToolResult(
            content=[TextContent(text=_with_header(header, output_text))],
            details={
                "command": command,
                "exit_code": exit_code,
                "timed_out": timed_out,
