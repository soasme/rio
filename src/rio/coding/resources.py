...
class ResourceError(ValueError):
    """Raised when rio resources are invalid or cannot be expanded."""


@dataclass(frozen=True, slots=True)
class SystemPromptResources:
    """Discovered rio-native system-prompt file contents and sources."""
...
@dataclass(frozen=True, slots=True)
class RioResourcePaths:
    """Filesystem locations for rio markdown resources.

    By default rio loads both rio-native resources and `.agents` resources from
    the user home directory. When a cwd is provided, project-local `.rio` and
    `.agents` resources are loaded automatically as well.
    """

    root: Path = field(default_factory=lambda: Path.home() / ".rio")
    cwd: Path | None = None
    ...
    def skills_dir(self) -> Path:
        """Return the primary rio skills directory."""
        return self.root / "skills"

    @property
    def prompts_dir(self) -> Path:
        """Return the primary rio prompt templates directory."""
    ...
    def themes_dirs(self) -> tuple[Path, ...]:
        ...
        Themes are rio-specific, so unlike skills and prompts no ``.agents``
    ...
    def _paths(self) -> RioPaths:
        agents_home = self.agents_root or Path.home() / ".agents"
        return self.paths or RioPaths(home=self.root, agents_home=agents_home)
...
def _discover_system_prompt_file(
    ...
    *,
    ...
    if paths.cwd is not None and paths.project_resources_enabled:
        candidates.append(("project", paths.cwd / ".rio" / filename))
...
def _discover_append_system_prompt_files(
    ...
    *,
    ...
    if paths.cwd is not None and paths.project_resources_enabled:
        candidates.append(("project", paths.cwd / ".rio" / "APPEND_SYSTEM.md"))
