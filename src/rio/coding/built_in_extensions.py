"""Trusted extension declarations bundled with Rio."""
...
from rio.coding.credentials import CredentialStore
from rio.coding.paths import RioPaths

if TYPE_CHECKING:
    from rio.coding.extensions.api import ExtensionAPI
...
class BuiltInExtensionContext:
    ...
    context exists so a bundled integration uses the session's Rio home,
...
class BuiltInExtension:
    """One trusted extension setup function shipped as part of Rio.

    Built-ins use the normal extension API and runtime lifecycle. They differ
    only in provenance: Rio declares their callable directly, loads them before
...
def _llama_cpp_setup(api: ExtensionAPI) -> None:
    ...
    from rio.coding.extensions.builtins.llama_cpp import setup
...
# project inputs cannot change what Rio treats as trusted.
