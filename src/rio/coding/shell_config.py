"""Durable shell execution settings for rio terminal commands."""
...
from rio.coding.project_trust import TrustDefault


class ShellConfigError(ValueError):
    """Raised when rio shell settings are invalid."""


@dataclass(frozen=True, slots=True)
class ShellSettings:
    """Shell execution settings loaded from rio home."""
...
def shell_settings_path(paths: RioPaths | None = None) -> Path:
    """Return the durable shell settings path."""
    return (paths or RioPaths()).home / "settings.json"


def load_shell_settings(paths: RioPaths | None = None) -> ShellSettings:
...
def shell_settings_from_json(data: dict[str, Any]) -> ShellSettings:
    ...
    # rio installation cannot prevent an older installation from starting.
