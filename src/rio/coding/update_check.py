"""Best-effort PyPI update checks for the rio CLI."""
...
from rio.ai.http import get_json
from rio.coding.paths import RioPaths

PYPI_PACKAGE_NAME = "rio"
...
UPDATE_CHECK_ENV_DISABLE = "RIO_NO_UPDATE_CHECK"
...
class UpdateNotice:
    ...
    def message(self) -> str:
        ...
        return (
            f"rio {self.latest_version} is available (installed: {self.current_version}). "
            "Run `rio update` to upgrade."
        )
...
class ReleaseNotesEntry:
    """Structured release notes for one rio version."""
...
class ReleaseNotesNotice:
    """Release notes shown once after the installed rio version changes."""
    ...
    def message(self) -> str:
        ...
        return f"rio updated to {self.current_version}\n\n{body}"
...
def startup_update_notice(
    ...
    *,
    ...
    """Return an update notice when PyPI has a newer stable rio release.
...
def startup_release_notes_notice(
    ...
    *,
    ...
    """Return release notes once after rio starts with a newer installed version.
...
def fetch_latest_pypi_version(*, fetcher: Fetcher | None = None) -> str | None:
    """Fetch the latest stable rio version from PyPI."""
...
def default_update_check_cache_path(paths: RioPaths | None = None) -> Path:
    """Return the on-disk cache path for startup update checks."""
    return (paths or RioPaths()).home / "cache" / "update-check.json"


def default_release_notes_state_path(paths: RioPaths | None = None) -> Path:
    """Return the on-disk state path for one-time release notes."""
    return (paths or RioPaths()).home / "cache" / RELEASE_NOTES_STATE_FILENAME
